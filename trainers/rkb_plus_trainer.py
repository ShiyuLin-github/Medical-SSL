...
class RKBPTrainer(BaseTrainer):
    ...
    def backward(self):
        ...
        self.rot_loss = self.bce_criterion(
            self.hor_rot_pred, self.hor_rot_label
        ) + self.bce_criterion(self.ver_rot_pred, self.ver_rot_label)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.epochs):
            ...
            for itr, sample in tqdm(enumerate(train_bar)):
                self.set_input(sample) # sample[0]是input，shape为[4, 1, 240, 240, 154]
                self.optimize_parameters() # model(input)部分在这里
                tloss.update(round(self.loss.item(), 2), self.config.train_batch)

                if (itr + 1) % 500 == 0:
                    self.recorder.logger.info(
                        "Epoch [{}/{}], iteration {}, Train Loss: {:.6f}".format(
                            epoch + 1, self.config.epochs, itr + 1, tloss.avg
                        )
                    )
                    sys.stdout.flush()
            ...
            with torch.no_grad():
                ...
                for itr, sample in enumerate(self.eval_dataloader):
                    ...
                    order_valid_acc += (
                        (predicted_order_label == self.order_label).sum().item()
                    )
                    total += self.order_label.size(0)

                    # print(self.hor_rot_pred.ge(0.5).int(), self.hor_rot_label)
                    hor_rot_valid_acc += (
                        (self.hor_rot_pred.ge(0.5).int() == self.hor_rot_label)
                        .sum()
                        .item()
                    )
                    # rot acc
                    ...
                    ver_rot_valid_acc += (
                        (self.ver_rot_pred.ge(0.5).int() == self.ver_rot_label)
                        .sum()
                        .item()
                    )
                    ...
                    # mask acc
                    mask_valid_acc += (
                        self.mask_pred.ge(0.5).int() == self.mask_label
                    ).sum()
            ...
            # logging
            self.recorder.logger.info(
                "Epoch {}, order acc is {:4f}, hor rot acc is {:.4f},"
                " ver rot acc is {:.4f}, mask acc is {:.4f}".format(
                    epoch + 1,
                    order_valid_acc,
                    hor_rot_valid_acc,
                    ver_rot_valid_acc,
            ...
            valid_acc = (
                order_valid_acc
                + (hor_rot_valid_acc + ver_rot_valid_acc) / 2
                + mask_valid_acc
            ) / 3
            if valid_acc > best_acc:
                self.recorder.logger.info(
                    "Validation acc increases from {:.4f} to {:.4f}".format(
                ...
                self.save_state_dict(
                    epoch + 1, os.path.join(self.recorder.save_dir, "RCB_MRI.pth")
                )
                self.recorder.logger.info(
                    "Saving model{} ".format(
                        os.path.join(self.recorder.save_dir, "RCB_MRI.pth")
                    )
                )
            else:
                self.recorder.logger.info(
                    "Validation acc does not increase from {:.4f}, num_epoch_no_improvement {}".format(
                        best_acc, num_epoch_no_improvement
                    )
                )
